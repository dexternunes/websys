...
public interface ReservaRepository extends RepositoryBaseRoot<Reserva> {
	...
	@Query("SELECT r FROM Reserva r WHERE r.inicioReserva = :inicioReserva AND r.excluido = 0 AND r.ativo = 1 AND r.grupo IN(:grupo)")
	Reserva getReservaByDate(@Param("inicioReserva") Date inicioReserva, @Param("grupo") List<Grupo> grupo);
}
